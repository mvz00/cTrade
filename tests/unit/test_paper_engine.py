...
class TestOrderExecution:
    ...
    async def test_market_buy_fills_immediately(self):
        engine = PaperEngine.get_instance()
        order = await engine.place_order("BTC/USDT", "buy", "market", 0.01)
    ...
    @pytest.mark.asyncio
    async def test_market_sell_fills_immediately(self):
        engine = PaperEngine.get_instance()
        # Buy first to have some BTC
        await engine.place_order("BTC/USDT", "buy", "market", 0.01)
        # Now sell
        order = await engine.place_order("BTC/USDT", "sell", "market", 0.01)
        assert order.status == "filled"
    ...
    async def test_insufficient_balance_rejected(self):
        ...
        order = await engine.place_order("BTC/USDT", "buy", "market", 100.0)
    ...
    async def test_sell_without_holdings_rejected(self):
        engine = PaperEngine.get_instance()
        order = await engine.place_order("BTC/USDT", "sell", "market", 1.0)
        assert order.status == "rejected"

    @pytest.mark.asyncio
    async def test_order_appears_in_list(self):
        engine = PaperEngine.get_instance()
        await engine.place_order("BTC/USDT", "buy", "market", 0.01)
    ...
    @pytest.mark.asyncio
    async def test_limit_order_stays_pending(self):
        engine = PaperEngine.get_instance()
        order = await engine.place_order("BTC/USDT", "buy", "limit", 0.01, price=50000.0)
        assert order.status == "pending"


class TestPositionManagement:
    @pytest.mark.asyncio
    async def test_buy_creates_long_position(self):
        engine = PaperEngine.get_instance()
        await engine.place_order("BTC/USDT", "buy", "market", 0.01)
    ...
    @pytest.mark.asyncio
    async def test_sell_closes_long_position(self):
        engine = PaperEngine.get_instance()
        await engine.place_order("BTC/USDT", "buy", "market", 0.01)
        await engine.place_order("BTC/USDT", "sell", "market", 0.01)
    ...
    @pytest.mark.asyncio
    async def test_close_position_by_id(self):
        engine = PaperEngine.get_instance()
        await engine.place_order("BTC/USDT", "buy", "market", 0.01)
        positions = engine.get_positions("open")
        pos_id = positions[0]["id"]
        order = await engine.close_position(pos_id)
    ...
    async def test_close_nonexistent_position(self):
        engine = PaperEngine.get_instance()
        result = await engine.close_position("nonexistent-id")
        assert result is None


class TestPortfolio:
    @pytest.mark.asyncio
    async def test_portfolio_after_buy(self):
        engine = PaperEngine.get_instance()
        await engine.place_order("BTC/USDT", "buy", "market", 0.01)
        portfolio = await engine.get_portfolio()
    ...
    @pytest.mark.asyncio
    async def test_portfolio_after_round_trip(self):
        engine = PaperEngine.get_instance()
        await engine.place_order("BTC/USDT", "buy", "market", 0.01)
        await engine.place_order("BTC/USDT", "sell", "market", 0.01)
        portfolio = await engine.get_portfolio()
    ...
    @pytest.mark.asyncio
    async def test_equity_curve_grows_with_trades(self):
        engine = PaperEngine.get_instance()
        await engine.place_order("BTC/USDT", "buy", "market", 0.01)
        curve = engine.get_equity_curve()
    ...
    @pytest.mark.asyncio
    async def test_recent_trades_after_close(self):
        engine = PaperEngine.get_instance()
        await engine.place_order("BTC/USDT", "buy", "market", 0.01)
        await engine.place_order("BTC/USDT", "sell", "market", 0.01)
        trades = engine.get_recent_trades()
